# app.py - Professional Gmail OTP API with Smaller OTP Box (Vercel Compatible)
import os
import re
import random
import smtplib
...
# ==========================================
# OTP STORAGE (In-memory - for serverless use external DB like Redis for production)
# ==========================================

class OTPStorage:
    """Thread-safe OTP storage - Note: For Vercel serverless, consider using Redis/Vercel KV"""
...
def create_rich_email_template(recipient: str, otp: str, expiry_minutes: int) -> str:
    ...
    <body style="margin: 0; padding: 0; font-family: 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Helvetica, Arial, sans-serif; background-color: #f0f2f5; line-height: 1.6;">
        ...
        <div style="max-width: 550px; margin: 20px auto; background-color: #ffffff; border-radius: 16px; overflow: hidden; box-shadow: 0 10px 25px rgba(0,0,0,0.1);">
            ...
            <div style="background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); padding: 30px 25px; text-align: center;">
                ...
                <h1 style="color: #ffffff; margin: 0; font-size: 24px; font-weight: 700; letter-spacing: -0.5px;">{config.COMPANY_NAME}</h1>
...
def send_otp(email):
    """
    Send OTP to email
    URL Format: /gmail=user@example.com
    """
    # Validate email
...
def verify_otp(email, otp_code):
    """
    Verify OTP
    URL Format: /gmail=user@example.com/check=123456
    """
    # Validate email
    valid, result = validate_email(email)
    if not valid:
...
def resend_otp(email):
    """
    Resend OTP to email
    URL Format: /resend/gmail=user@example.com
    """
    # Validate email
    valid, result = validate_email(email)
    if not valid:
...
def check_status(email):
    """
    Check OTP status
    URL Format: /status/gmail=user@example.com
    """
    # Validate email
    valid, result = validate_email(email)
    if not valid:
...
if __name__ == '__main__':
    print("\n" + "="*60)
    print(f"🚀 {config.COMPANY_NAME} - PROFESSIONAL GMAIL OTP API")
    print("="*60)
    print(f"\n✅ Server running on: http://localhost:5000")
    print(f"✅ Company: {config.COMPANY_NAME}")
    print(f"✅ Logo URL: {config.COMPANY_LOGO_URL}")
    print("\n📌 API ENDPOINTS:")
    print("-"*40)
    print("\n1️⃣  Send OTP (Rich Email):")
    print("   GET /gmail=user@example.com")
    print("\n2️⃣  Verify OTP:")
    print("   GET /gmail=user@example.com/check=123456")
    print("\n3️⃣  Resend OTP:")
    print("   GET /resend/gmail=user@example.com")
    print("\n4️⃣  Check Status:")
    print("   GET /status/gmail=user@example.com")
    print("\n" + "="*60 + "\n")
